...
The project produced both expected confirmations and genuine mathematical surprises. The monoidal structure on $\CondAb$ turned out to already exist in Mathlib (we just had to find it). The embedding from seminormed groups turned out not to be full: a counterexample found while attempting to formalize fullness shows the obstruction, and we subsequently machine-checked it at the presheaf level. Both findings reshaped the project's direction in real time.
...
The discreteness of $V$ is the engine: the condensed (here, presheaf) setting sees \emph{all} additive maps as morphisms, while $\SemiNormedGrp$ admits only bounded ones, so the embedding cannot be full. The entire development is sorry-free and uses no custom axioms.

\textbf{From presheaves to sheaves.} Because morphisms of sheaves coincide with morphisms of presheaves between sheaf-valued objects, the non-fullness of $E$ captures the full obstruction for the genuine functor into $\CondAb$. Lifting Theorem~\ref{thm:fullness} to a single sheaf-level statement requires only identifying $S \mapsto C(S,A)$ as a sheaf for the coherent topology, which the current Mathlib version does not provide. We therefore prove faithfulness at the sheaf level and the sharper faithful-but-not-full statement at the presheaf level, with the gap between them isolated to this one missing lemma.
...
\bottomrule
\end{tabular}
\caption{Theorem inventory for the machine-checked development. The faithful-but-not-full result (Theorem~\ref{thm:fullness}) is verified at the presheaf level; the sheaf-level lift requires one additional lemma.}
\label{tab:results}
\end{table}
...
\textbf{Finding 3.} Fullness fails for the embedding (Theorem~\ref{thm:fullness}), machine-checked at the presheaf level. The condensed setting sees strictly more morphisms than the normed category, because the sup-norm topology on integer sequences is discrete and admits unbounded continuous additive maps.
...
Six rounds of AI-assisted formal verification, starting from a blank slate, produced approximately 1380 active lines of Lean~4 across 5 files with 5 sorries and 0 custom axioms. The central results: $\CondAb$ is a sorry-free symmetric monoidal abelian category, the TQFT transfer theorem is sorry-free, the embedding from seminormed groups exists sorry-free with the sheaf condition verified, the presheaf-level embedding is machine-checked to be faithful but not full, and the commutative Frobenius algebra structure classifying 2d TQFTs~\cite{Kock2003} is sorry-free.

A notable result was machine-checking that the embedding, presented as a functor $E$ into presheaves on $\mathrm{CompHaus}$, is faithful but not full. This is a precise mathematical boundary: the condensed setting sees more morphisms between normed groups than the normed category itself does, because the sup-norm topology on integer sequences is discrete and admits unbounded continuous additive maps. Faithfulness holds at the sheaf level as well; lifting the non-fullness to the sheaf level requires only identifying $S \mapsto C(S,A)$ as a sheaf, the one piece absent from the current Mathlib.
