...
The project produced both expected confirmations and genuine mathematical surprises. The monoidal structure on $\CondAb$ turned out to already exist in Mathlib (we just had to find it). The embedding from seminormed groups turned out not to be full: an elementary counterexample, found while attempting to formalize fullness, shows the obstruction. (This counterexample is an ordinary mathematical argument; we did not formalize it in Lean.) Both findings reshaped the project's direction in real time.

\subsection{Navigating the Mathematical Landscape}

To contextualize where this work sits within modern mathematics, we developed MathFlowChart, an interactive dependency map of 221 mathematical concepts and 491 connections (available as supplementary material). The chart visualizes the path from foundational structures through the Scholze program to TQFT, tracking the Langlands program connections and chromatic homotopy theory bridges.
...
\subsection{Session 5: Properties and a Counterexample}

\begin{theorem}[Sorry-free]
The embedding $\SemiNormedGrp \to \CondAb$ is faithful.
\end{theorem}
...
\begin{proposition}[Not formalized]
\label{thm:fullness}
The embedding $\SemiNormedGrp \to \CondAb$ is \textbf{not} full.
\end{proposition}

\begin{proof}
Let $V = c_0(\mathbb{N}, \mathbb{Z})$ (eventually-zero integer sequences with sup norm) and $W = \mathbb{Z}$. The summation map $f(a) = \sum a_n$ is additive and continuous (since $\|a\| < 1$ implies $a = 0$ for integer sequences), but not bounded ($\|e_1 + \cdots + e_n\| = 1$ while $f(e_1 + \cdots + e_n) = n$). Postcomposition by~$f$ defines a valid condensed morphism with no lift to $\SemiNormedGrp$.
\end{proof}

We state this as an ordinary mathematical proposition rather than a Lean theorem: unlike the faithfulness result, the failure of fullness is recorded as a paper argument and is not part of the machine-checked development.

\textbf{Root cause:} continuous additive maps between seminormed abelian groups need not be bounded. The standard proof that ``continuous linear implies bounded'' requires scalar multiplication by a dense subfield. Fullness holds when restricted to $\mathrm{NormedSpace}\;R$ for~$R$ a nontrivially normed field.

The session also constructed $\SemiNormedGrp.\mathrm{hasFiniteProducts}$ (sorry-free, not previously in Mathlib).
...
Embedding is faithful & BanachEmbedding.lean & Sorry-free \\
Embedding is full & BanachEmbedding.lean & Disproved (on paper) \\
...
\bottomrule
\end{tabular}
\caption{Theorem inventory for the machine-checked development. The failure of fullness (Proposition~\ref{thm:fullness}) is a paper argument, not a Lean result, and is listed separately.}
\label{tab:results}
\end{table}
...
\textbf{Finding 3.} Fullness fails for $\SemiNormedGrp$ (Proposition~\ref{thm:fullness}, an elementary argument we did not formalize). The condensed category sees strictly more morphisms than the normed category.
...
%% ============================================================
\section{The MathFlowChart}
\label{sec:chart}
%% ============================================================

To contextualize the mathematical dependencies, we developed MathFlowChart: an interactive HTML/SVG/React diagram mapping 221 nodes and 491 edges across modern mathematics, from categorical foundations through the Scholze program to TQFT. The chart is available as supplementary material.
...
Six rounds of AI-assisted formal verification, starting from a blank slate, produced approximately 1160 active lines of Lean~4 across 4 files with 5 sorries and 0 custom axioms. The central results: $\CondAb$ is a sorry-free symmetric monoidal abelian category, the TQFT transfer theorem is sorry-free, the embedding from seminormed groups exists sorry-free with the sheaf condition verified, and the commutative Frobenius algebra structure classifying 2d TQFTs~\cite{Kock2003} is sorry-free.

A notable byproduct was identifying that the embedding $\SemiNormedGrp \to \CondAb$, while faithful (proved in Lean), is not full. This is a precise mathematical boundary: condensed abelian groups see more morphisms between normed groups than the normed category itself does, because continuous additive maps need not be bounded without scalar structure. We record this as an elementary argument rather than a formalized result; fullness is recovered by restricting to normed vector spaces over a valued field.
...
The complete Lean~4 formalization is available at \url{https://github.com/seanm27lol/Liquid-tft-lean-CM4CM-} and builds against Mathlib v4.28.0. The MathFlowChart interactive dependency map is included as supplementary material.
